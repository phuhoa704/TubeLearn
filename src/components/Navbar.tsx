...
export const Navbar = ({ setCollapsed, collapsed }: Props) => {
    ...
    <header className="h-16 min-h-16 bg-surface-main border-b border-border-main flex items-center px-4 gap-2 shrink-0 z-20">
      ...
      <div className="relative" ref={themeRef}>
        <Button
          ...
          className={`w-9 h-9 rounded-r1  flex items-center justify-center text-text-sub cursor-pointer hover:bg-surface-alt hover:border-border-alt transition-colors duration-150 shadow-sm p-0!`}
          title="테마 색상"
        >
          <svg
            width="15"
        ...
        <div
          className={`absolute top-full right-0 mt-2 bg-surface-main border border-border-main rounded-r2 shadow-sh3 p-2.5 w-47.5 z-50 transition-all duration-150 ${
            themeOpen
            ...
            {themeSwatches.map((t) => (
              <button
                ...
                className={`flex items-center gap-2.5 py-1.5 px-2 rounded-r1 text-xs font-semibold w-full text-left transition-colors duration-100 hover:bg-surface-alt ${
                  theme === t.id ? "text-text-main font-bold" : "text-text-sub"
                }`}
              >
                <span
                  className={cn(
      ...
      <div className="flex items-center bg-surface-alt border border-border-main rounded-full gap-0.5 p-0.75">
        <button
          onClick={() => mode !== "light" && toggleMode()}
          className={cn(
            "w-7 h-7 rounded-full flex items-center justify-center cursor-pointer transition-all duration-150",
            mode === "light"
              ? "bg-surface-main text-text-main shadow-sh1"
      ...
      <Button
        variant="ghost"
        className={`w-9 h-9 rounded-r1 relative flex items-center justify-center text-text-sub cursor-pointer hover:bg-surface-alt hover:border-border-alt transition-colors duration-150 shadow-sm p-0!`}
      >
        <svg
          width="15"
          height="15"
        ...
        <div className="absolute top-1 right-1 w-4 h-4 bg-err rounded-full text-white font-bold text-[9px] flex items-center justify-center border border-surface-main">
          6
        </div>
      </Button>
